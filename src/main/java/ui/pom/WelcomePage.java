package ui.pom;

import com.microsoft.playwright.Locator;
import com.microsoft.playwright.Page;
import lombok.extern.slf4j.Slf4j;
import ui.pom.utils.LogHelper;

@Slf4j
public class WelcomePage extends BaseUiTest implements BasePage
{

	private Locator yourFeedButton;
	private Locator newArticleButton;
	private Locator authorsList;
	private Locator selectedTag;
	private Page page;

	public WelcomePage(Page page)
	{
		if (page == null)
		{
			LogHelper.error("Page object cannot be null");
			throw new IllegalArgumentException("Page object cannot be null");
		}
		this.page = page;
		this.yourFeedButton = page.locator("//button[text()='Your Feed']");
		this.newArticleButton = page.locator("//a[text()='New Article']");
		this.authorsList = page.locator("//a[@class = 'author']");
		this.selectedTag = page.locator("//button/i");
		LogHelper.debug("Your Feed button locator initialized");
	}

	@Override
	public boolean isAtCorrectPage()
	{
		waitForNetworkIdle(page);
		boolean isVisible = waitForVisibility(yourFeedButton);
		if (isVisible)
		{
			LogHelper.debug("Your Feed button is visible. User is at the correct page");
		}
		else
		{
			LogHelper.warn("Your Feed button is not visible. User might not be at the correct page");
		}
		return isVisible;
	}

	public void clickNewArticle()
	{
		clickOnElement(newArticleButton);
	}

	public void selectTagByNumber(int tagNumber)
	{
		clickOnElement(page.locator("//div[@class = 'tag-list']/button[" + tagNumber + "]"));
	}

	public void selectTagByName(String tagName)
	{
		clickOnElement(page.locator("//div[@class = 'tag-list']/button", new Page.LocatorOptions().setHasText(tagName)));
	}

	public void selectFirstAuthor()
	{
		clickOnElement(authorsList);
	}

	public String getFirstAuthorName()
	{
		return getElementText(authorsList);
	}

	public boolean isTagPresent(String tagToSearch)
	{
		if (!waitForVisibility(selectedTag))
		{
			log.error("The selected tag is not visible");
			return false;
		}
		if (getElementText(selectedTag).equals(tagToSearch))
		{
			log.error("The current tag is not the selected one");
			return false;
		}
		return true;
	}

}